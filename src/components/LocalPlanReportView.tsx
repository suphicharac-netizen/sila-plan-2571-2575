...
export const LocalPlanReportView: React.FC<LocalPlanReportViewProps> = ({
  ...
  const getEditionTitleLine = () => {
    if (activeTab === 'p01') {
      return 'แผนพัฒนาท้องถิ่น (พ.ศ. 2571-2575)';
    }
    if (activeTab === 'p02_first' || activeTab === 'all_summary') {
      return 'แผนพัฒนาท้องถิ่น (พ.ศ. 2571-2575)';
    }
    ...
    const pWithEd =
      filteredProjects.find((p) => p.edition === targetEditionType && p.editionNumber) ||
      projects.find((p) => p.edition === targetEditionType && p.editionNumber);
  ...
  // Standard P.02 table header renderer
  const renderP02TableHeader = (showReason: boolean) => (
    <thead>
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 h-full min-h-0 overflow-hidden print:bg-white print:overflow-visible">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto print:overflow-visible">
        {/* 2. Controls & Filter Bar Container (Hidden on Print) */}
        <div className="p-3 sm:p-4 space-y-3 no-print print:hidden">
          <div className="bg-white border border-slate-200 rounded-xl p-4 sm:p-5 shadow-xs space-y-4">
            {/* แถวบนสุด: Dropdown ปีงบประมาณ: เลือก "ทั้งหมด (2571-2575)" */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="ค้นหาชื่อโครงการในรายงาน..."
                    value={searchQuery}
            ...
            {/* แถบปุ่มแอ็กชันและปุ่มสลับประเภทฉบับแผน (Action Bar): */}
            <div className="pt-2 flex flex-col lg:flex-row lg:items-center justify-between gap-3 border-t border-slate-100">
              {/* ฝั่งซ้าย: ปุ่ม 'ค้นหา' (สีเขียว + ไอคอนแว่นขยาย), ปุ่ม 'แสดงทั้งหมด' (สีขาวขอบเทา), ปุ่ม 'เริ่มใหม่' (สีขาวขอบส้ม) */}
              <div className="flex flex-wrap items-center gap-2">
                <button
                  type="button"
                  onClick={() => {}}
                  className="inline-flex items-center gap-1.5 px-4 py-2 bg-[#059669] hover:bg-[#047857] text-white text-xs font-semibold rounded-lg shadow-xs transition-colors cursor-pointer"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-white hover:bg-amber-50/50 text-amber-700 border border-amber-300 text-xs font-medium rounded-lg shadow-xs transition-colors cursor-pointer"
                >
              ...
              {/* ฝั่งขวา (ปุ่มสลับสไตล์ Tab): แบบ ผ.01 | แบบ ผ.02 (ฉบับแรก) | แบบ ผ.02 (เพิ่มเติม) | แบบ ผ.02 (เปลี่ยนแปลง) | แบบ ผ.02 (แก้ไข) | ฉบับรวม */}
        ...
        <div className="px-4 sm:px-6 pb-12 print:p-0 print:m-0">
          ...
          >
            ...
            {/* Centered Document Header - Standard 3-Line Official Format */}
            <div className="text-center space-y-1 mb-6">
              {/* บรรทัดที่ 1 (ตัวหนา): บัญชีรายละเอียดโครงการพัฒนาท้องถิ่น */}
              ...
              {/* บรรทัดที่ 2 (ตัวหนา): แสดงชื่อประเภทฉบับตามบริบท */}
              <h3 className="text-sm sm:text-base font-bold text-slate-900 print:text-black leading-tight">
                {getEditionTitleLine()}
              </h3>

              {/* บรรทัดที่ 3: เทศบาลเมืองศิลา อำเภอเมืองขอนแก่น จังหวัดขอนแก่น */}
              <p className="text-xs sm:text-sm text-slate-800 print:text-black leading-tight">
              ...
              {Object.keys(groupedByStrategy).length === 0 ? (
                ...
                <div className="space-y-6">
                  {Object.entries(groupedByStrategy).map(([strategyTitle, prjList], stIdx) => {
                    ...
                    return (
                      <div key={strategyTitle} className="strategy-section">
                        {/* 1. หัวข้อกลุ่มประเด็นการพัฒนา (Group Header เหนือก่อนเริ่มตารางโครงการเสมอ) */}
                        <div className="strategy-section-header text-slate-900 print:text-black">
                          {stIdx + 1}. ประเด็นการพัฒนาท้องถิ่น: {strategyTitle}
                        </div>

                        {/* 2. ตารางโครงการของประเด็นการพัฒนานี้ */}
                        <div className="overflow-x-auto">
                          <table className="report-table w-full border-collapse border border-slate-900 text-xs p02-print-table">
                            {renderP02TableHeader(showReasonColumn)}
                            ...
                            <tfoot className="border-t-2 border-slate-900 font-bold">
                              <tr className="bg-slate-100/80 print:bg-slate-50 font-bold text-slate-900 print:text-black border-t-2 border-b border-slate-900">
                                <td colSpan={4} className="border border-slate-900 py-2.5 px-3 text-right font-bold text-xs print:border-black print:text-black">
                                  รวม {prjList.length} โครงการ
                                </td>
                                <td className="border border-slate-900 py-2.5 px-2 text-right font-mono text-xs font-bold print:border-black print:text-black">
                                ...
                                <td
                                  ...
                                  className="border border-slate-900 py-2.5 px-3 print:border-black print:text-black"
                                />
                              </tr>
                            </tfoot>
                          </table>
                  ...
                  {/* สรุปรวมทั้งสิ้นท้ายรายงาน (Grand Total Summary) */}
                  <div className="overflow-x-auto mt-6 print:mt-6">
                    <table className="report-table w-full border-collapse border border-slate-900 text-xs p02-print-table">
                      <tfoot className="border-t-2 border-b-2 border-slate-900 font-bold">
                        <tr className="bg-slate-100 print:bg-slate-100 font-bold text-slate-900 print:text-black border-t-2 border-b-2 border-slate-900">
                          {/* ช่องซ้าย (Colspan ยุบรวมช่องที่ 1 ถึง 4): ชิดขวา ตัวหนา */}
                          <td
                            colSpan={4}
                            className="border border-slate-900 py-3 px-3 text-right font-bold text-xs print:border-black print:text-black"
                          >
                            รวมทั้งสิ้น {filteredProjects.length} โครงการ
                          </td>
                          {/* ช่องงบประมาณ 5 ช่อง (พ.ศ. 2571 - 2575): แสดงยอดรวมงบประมาณทั้งสิ้นแยกตามแต่ละปี พ.ศ. (ตัวหนา) */}

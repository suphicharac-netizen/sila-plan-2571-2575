...
export interface ProjectTrackingItem {
  ...
  'ยอดตั้งต้น'?: number; // Initial Approved Budget (ยอดตั้งต้น / อนุมัติเดิม)
  'โอนเพิ่ม'?: number; // Budget Transferred In (โอนเพิ่ม (+))
  'โอนลด'?: number; // Budget Transferred Out (โอนลด (-))
  'งบอนุมัติสุทธิ'?: number; // Net Approved Budget (งบอนุมัติสุทธิ)
